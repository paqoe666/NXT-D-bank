...
import { LogOut, CreditCard, History, Settings, Search, ArrowUpRight, ArrowDownLeft, X, FileText, CheckCircle2, XCircle, Trash2, RefreshCw } from 'lucide-react';
import { useNavigate, useLocation } from 'react-router-dom';

const translations = {
  ru: { dash: 'Главная', hist: 'Операции', set: 'Настройки', title: 'История операций', income: 'Пополнения и доходы', expense: 'Расходы и переводы', all: 'Все', incomes: 'Доходы', expenses: 'Расходы', searchText: 'Поиск операции...', listTitle: 'Список транзакций', notFound: 'Операции не найдены', clearHistory: 'Очистить историю', confirmClear: 'Вы уверены, что хотите удалить всю историю транзакций?', unknown: 'Неизвестный получатель', err: 'ОШИБКА', comm: 'Ком.', simNote: 'Учебный симулятор', success: 'Успешно', fail: 'Возврат / Ошибка', opId: 'ID Операции', dateTime: 'Дата и время', sender: 'Отправитель', receiver: 'Получатель', commission: 'Комиссия', comment: 'Комментарий', close: 'Закрыть', ceoDeposit: 'Пополнение счета CEO', repeat: 'Повторить перевод' },
  en: { dash: 'Dashboard', hist: 'History', set: 'Settings', title: 'Transaction History', income: 'Deposits & Income', expense: 'Expenses & Transfers', all: 'All', incomes: 'Income', expenses: 'Expenses', searchText: 'Search transaction...', listTitle: 'Transactions List', notFound: 'No transactions found', clearHistory: 'Clear history', confirmClear: 'Are you sure you want to delete all transaction history?', unknown: 'Unknown recipient', err: 'ERROR', comm: 'Fee', simNote: 'Training Simulator', success: 'Success', fail: 'Refund / Error', opId: 'Operation ID', dateTime: 'Date & Time', sender: 'Sender', receiver: 'Receiver', commission: 'Fee', comment: 'Comment', close: 'Close', ceoDeposit: 'CEO Account Deposit', repeat: 'Repeat transfer' },
  es: { dash: 'Inicio', hist: 'Operaciones', set: 'Ajustes', title: 'Historial de Operaciones', income: 'Ingresos y Depósitos', expense: 'Gastos y Transferencias', all: 'Todo', incomes: 'Ingresos', expenses: 'Gastos', searchText: 'Buscar transacción...', listTitle: 'Lista de Transacciones', notFound: 'No se encontraron operaciones', clearHistory: 'Borrar historial', confirmClear: '¿Estás seguro de que deseas eliminar todo el historial de transacciones?', unknown: 'Destinatario desconocido', err: 'ERROR', comm: 'Com.', simNote: 'Simulador de Entrenamiento', success: 'Éxito', fail: 'Reembolso / Error', opId: 'ID de Operación', dateTime: 'Fecha y Hora', sender: 'Remitente', receiver: 'Destinatario', commission: 'Comisión', comment: 'Comentario', close: 'Cerrar', ceoDeposit: 'Depósito a cuenta CEO', repeat: 'Repetir transferencia' }
};
...
export default function Operations() {
  ...
  // Функция для повторения перевода
  ...
  const formatDate = (dateString: string) => {
    return new Date(dateString).toLocaleDateString(language === 'ru' ? 'ru-RU' : language === 'es' ? 'es-ES' : 'en-US', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' });
  };
              ...
              <div className="p-5 space-y-3 text-xs bg-slate-50 dark:bg-slate-800/50">
                ...
                {selectedTx.comment && (
                  <div>
                    ...
                    <span className="font-italic bg-white dark:bg-slate-900 p-2 rounded-lg block border border-slate-100 dark:border-slate-700 text-slate-900 dark:text-white">"{selectedTx.comment}"</span>
                  </div>
                )}
              </div>
              
              {/* КНОПКА ПОВТОРИТЬ ПЕРЕВОД (только для исходящих) */}
              {selectedTx.senderId === myId && selectedTx.type !== 'deposit' && (
                <div className="p-4 bg-white dark:bg-slate-800">
                  <button 
                    onClick={() => handleRepeatTransfer(selectedTx)}
                    className="w-full py-3 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-xl font-bold flex items-center justify-center gap-2 transition"
                  >
