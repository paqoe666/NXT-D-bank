...
// --- ХЕЛПЕР ДЛЯ ОПРЕДЕЛЕНИЯ ПЛАТЕЖНОЙ СИСТЕМЫ С ИНДИВИДУАЛЬНЫМ РАЗМЕРОМ ---
  const getCardSystem = (cardNumber: string) => {
    ...
    if (cleanNum.startsWith('7777')) return { name: 'N-CARD', logo: '/ncards.svg', style: 'bg-blue-600 text-white', icon: '', customClass: 'h-12 md:h-16 transform scale-100 origin-right' };
